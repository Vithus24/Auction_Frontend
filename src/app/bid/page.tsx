...
const getTeamImageUrl = (auction: Team) => {
  if (auction.imageUrl) {
    return `data:image/jpeg;base64,${auction.imageUrl}`;
  }
  return null;
};
...
export default function AuctionPage() {
  ...
  const fetchTeams = async (signal: AbortSignal): Promise<Team[]> => {
    ...
    return (rawTeams || []).map(
      (t: any): Team => ({
        id: Number(t.id ?? 0),
        name: String(t.name ?? t.teamName ?? "Unknown Team"),
        imageUrl: String(t.imageUrl ?? ""),
        playerCount: Number(t.playersCount ?? 0),
        maxBudget: Number(t.maxBudget ?? 0),
        remainingBudget: Number(t.remainingBudget ?? 0),
        currentBid: Number(t.currentBid ?? 0),
      })
    );
  };
  ...
  const fetchPlayers = async (signal: AbortSignal): Promise<Player[]> => {
    ...
    return (rawPlayers || []).map((p: any): Player => {
      const roleRaw = String(
        p.role ?? p.playerRole ?? p.category ?? p.position ?? "UNKNOWN"
      ).toUpperCase();
      const statusRaw = String(
        p.status ?? p.auctionStatus ?? p.playerStatus ?? "available"
      ).toLowerCase();
      ...
      //   typeof p.age === "number"
      //     ? `${p.age} Yrs`
      //     : typeof p.age === "string" && p.age
      //     ? p.age.toLowerCase().includes("yr")
      //       ? p.age
      //       : `${p.age} Yrs`
      //     : "N/A";
      ...
      return {
        ...
        lastname: String(p.lastname ?? "Unknown"),
        role: String(p.role ?? "Unknown"),
        age: Number(p.age ?? 0).toString(),
        photo: String(p.photoUrl ?? ""),
        status: statusRaw as Player["status"],
        basePrice: Number(p.basePrice ?? p.minimumBid ?? p.startPrice ?? 0),
        team: String(p.teamName ?? p.team ?? p.assignedTeam ?? ""),
        soldPrice: Number(p.soldPrice ?? p.finalPrice ?? p.winningBid ?? 0),
      };
    });
  };
            ...
            <div className="bg-gradient-to-br from-yellow-800 to-yellow-700 margin-0 pb-3 rounded-xl border border-blue-400">
              ...
              <div className="flex items-start space-x-3 pl-3 mb-1">
                ...
                <div className="flex-1">
                  ...
                  <div className="space-y-0.1 text-xs text-gray-200 leading-snug">
              ...
              <div className="bg-gradient-to-r from-yellow-600 to-gray-800 rounded-lg p-3 ml-3 mr-3 mb-3 flex items-center justify-between">
                <div className="flex items-center space-x-2">
                  <span className="text-white font-bold text-sm uppercase">
                    Current Price
                  </span>
              ...
              <div className="text-center text-white font-semibold tracking-wide uppercase text-sm border-t border-gray-700 pt-1">
                {currentPlayer.team || "—"}
              </div>
            </div>
          )}
                    ...
                    {(["all", "available", "sold", "unsold"] as const).map(
                      (filter) => (
                        <button
                          ...
                          className={`px-3 py-2 rounded-lg font-medium text-sm transition-colors hover:scale-105 ${
                            activeFilter === filter
                              ? "bg-blue-300 text-white"
                              : "bg-blue-800 text-white hover:bg-blue-600"
                          }`}
                ...
                <div className="flex items-center space-x-1">
                  ...
                  {roleFilters.map((role) => (
                    <button
                      key={role}
                      onClick={() => setActiveRoleFilter(role)}
                      className={`px-3 py-1 rounded text-sm font-medium transition-colors hover:scale-105 border border-blue-400 ${
                        activeRoleFilter === role
                          ? "bg-blue-300 text-white"
                          : "bg-blue-800 text-white hover:bg-blue-600"
                      }`}
                ...
                <div className="grid grid-cols-2 gap-4">
                  {teams.length === 0 ? (
                    <div className="col-span-2 text-center text-white py-8">
                      <div className="text-white-400">No teams found</div>
                    </div>
                  ) : (
                    teams.map((team) => (
                      <div
                    ...
                    {filteredPlayers.length === 0 ? (
                      ...
                      filteredPlayers.map((player, index) => (
                        ...
                        >
                          ...
                          {/* Status Badge */}
                          <div
                            className={`absolute top-0 right-0 text-white text-xs font-bold rounded-tr-lg px-2 py-1 shadow ${
                              player.status === "sold"
                                ? "bg-green-600"
                                : player.status === "unsold"
                                ? "bg-red-600"
                          ...
                          {/* Player Info */}
                          <div className="flex items-start space-x-3 p-3 pt-6">
                            {getPlayerImageUrl(player) ? (
                              <div className="w-16 h-16 rounded-xl overflow-hidden border-2 border-gray-300 shadow-md">
                                <img
                              ...
                              <div className="w-16 h-16 rounded-xl bg-gradient-to-br from-gray-200 to-gray-300 flex items-center justify-center border-2 border-gray-300 shadow-md">
                                <ImageIcon className="w-8 h-8 text-gray-500" />
                              </div>
                            )}
                            <div className="flex-1">
                          ...
                          {/* Role Badge */}
                          <div className="absolute bottom-3 left-2 text-xs font-bold text-orange-300 uppercase">
                            {player.role}
                          </div>

                          {/* Price/Team Bar */}
                          <div
                            className={`absolute bottom-0 left-0 w-full text-xs font-bold ${
                              player.status === "sold"
